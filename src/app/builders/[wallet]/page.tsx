...
export default function BuilderPage({ params }: { params: Promise<{ wallet: string }> }) {
  ...
  if (!data) return <p className="text-white/50">Loading builder…</p>;
  ...
  return (
    <div className="space-y-8">
      <header className="flex flex-wrap items-start gap-5">
        <div className="hud flex h-20 w-20 items-center justify-center rounded-sm border border-line bg-ink-2"><span className="h-6 w-6 rotate-45 bg-ember" /></div>
        <div className="min-w-0 flex-1">
          <h1 className="text-2xl font-bold">{p?.name || "Unnamed builder"}</h1>
          <a className="font-mono text-xs text-white/50 hover:text-white" href={explorerUrl(cfg, "address", wallet)} target="_blank" rel="noreferrer">{shortKey(wallet, 8)}</a>
          {p?.bio && <p className="mt-2 max-w-2xl whitespace-pre-wrap text-bone/80">{p.bio}</p>}
          {links.length > 0 && (
            <div className="mt-3 flex gap-2">{links.map((l) => <a key={l.label} href={l.href} target="_blank" rel="noreferrer" className="rounded-full border border-line px-3 py-1 text-xs hover:border-lime/60">{l.label} ↗</a>)}</div>
          )}
        </div>
      ...
      {editing && (
        <section className="grid gap-3 rounded-md border border-line bg-ink-2 p-5 sm:grid-cols-2">
      ...
      <section>
        ...
        {active.length ? <div className="grid gap-4 md:grid-cols-2">{active.map((l) => <ListingCard key={l.id} l={l} />)}</div> : <p className="text-white/50">Nothing listed right now.</p>}
      </section>
      {past.length > 0 && (
        <section>
