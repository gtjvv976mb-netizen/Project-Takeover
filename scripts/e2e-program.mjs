...
const listing = await post("/api/listings", seller, "create", null, {
  type: "token_authority", title: "E2E on-chain token", description: "settled by the program", priceSol: 2,
...
await sp.methods.createListing(idBytes(listing.id), { tokenAuthority: {} }, new BN(2 * LAMPORTS_PER_SOL), 1 | 2, 7)
...
const expected = 2 * LAMPORTS_PER_SOL * (1 - cfg.feeBps / 10000);
...
const off = await post("/api/listings", seller, "create", null, {
  type: "offchain", title: "E2E escrowed project", description: "tests the refund crank", priceSol: 1,
  asset: { category: "project", links: [], deliverables: "repo + domain" },
});
await sp.methods.createListing(idBytes(off.id), { offchain: {} }, new BN(LAMPORTS_PER_SOL), 0, 1)
...
if (afterRelease - beforeRelease !== LAMPORTS_PER_SOL * (1 - cfg.feeBps / 10000)) throw new Error("release payout wrong");
